...
-----------------------------------------]]
--[[Globals]]
IS_REAL_GAME = player.buffCount ~= nil
FramesPerSecond = 0
...
function item.ElexirOver(id)
    if not IS_REAL_GAME then return false end --debug
...
function getItemList()
    ...
    if map == "Summoners Rift" and isJungling() and Champions[player.charName].Jungler then
        ...
        if Champions[player.charName].Jungler.ItemList2 then itemsNotToSell = Champions[player.charName].Jungler.ItemList2 end
        if Champions[player.charName].Jungler.ItemList3 then tools = Champions[player.charName].Jungler.ItemList3 end
    elseif  Champions[player.charName][map] then
        ...
        if Champions[player.charName][map].ItemList2 then itemsNotToSell = Champions[player.charName].Jungler.ItemList2 end
        if Champions[player.charName][map].ItemList3 then tools = Champions[player.charName].Jungler.ItemList3 end
    end
    if itemsToBuy and itemsNotToSell and tools then
        for i, v in ipairs(itemsToBuy) do
            if item.ID(v) then table.insert(ItemList1, item.ID(v))
            else print("Error: " .. (item.Name(v) or (type(v)=="number" and v) or "?") .. " is not a valid item") end
        end
        for i, v in ipairs(itemsNotToSell) do
            if item.ID(v) then table.insert(ItemList2, item.ID(v))
			else print("Error: " .. v .. " is not a valid item") end
        end
        for i, v in ipairs(tools) do
            ItemList3[i] = v
            if item.ID(v[1]) then ItemList3[i][1] = item.ID(v[1])
            else print("Error: " .. (item.Name(v[1]) or (type(v[1])=="number" and v[1]) or "?") .. " is not a valid item") return false end
        end
